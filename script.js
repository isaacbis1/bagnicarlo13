...
// Riferimento doc "admin/dictionary"
...
document.addEventListener('DOMContentLoaded', () => {
  // 1) Carichiamo SUBITO tutti i user + admin in Firestore
  uploadAllUsersToFirestore();
  
  // 2) Carichiamo in realtime il dictionary
  loadDictionaryRealtime();
  
  // 3) Carichiamo le immagini di login
  loadAdminImagesRealtime();
  
  // 4) All'avvio mostriamo solo la sezione Access
...
function uploadAllUsersToFirestore() {
  db.collection("admin").doc("dictionary").set(dictionaryData)
    .then(() => {
...
function loadDictionaryRealtime() {
  dictionaryRef.onSnapshot(docSnap => {
    if (docSnap.exists) {
      ...
      console.log("Dictionary caricato in realtime:", dictionaryDataLive);
      if (isAdmin) {
...
function adminLogin() {
  ...
  if (!dictionaryDataLive["admin"]) {
    showNotification("Dictionary non caricato o 'admin' assente!");
    return;
  }
...
function userLoginDictionary() {
  ...
  // Salviamo i dati in registrations
...
/*****************************************
 *  PRENOTAZIONI (una al giorno se user)
 *****************************************/
let reservations = {
  "Volley1": {},
  "Volley2": {},
...
function resetAllReservations() {
  ...
  db.collection("reservations")
    ...
    .then(() => {
      showNotification("Prenotazioni reset per il nuovo giorno (admin escluso).");
      loadReservationsFromFirestore();
    })
    .catch(err => {
