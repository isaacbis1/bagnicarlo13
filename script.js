...
/*****************************************
 *  ALL'AVVIO - Carica SUBITO i 200 user
 *****************************************/
document.addEventListener('DOMContentLoaded', () => {
  toggleSections(false); 
  // Carica TUTTI i 200 user + admin in Firestore
  uploadAllUsersToFirestore(); 
  // Carica dictionary in realtime
  loadDictionaryRealtime(); 
  // Carica immagini login
  loadAdminImagesRealtime(); 
});

/*****************************************
 *  FUNZIONE: uploadAllUsersToFirestore()
 *****************************************/
...
function loadDictionaryRealtime() {
  dictionaryRef.onSnapshot(docSnap => {
    ...
    console.error("Errore dictionary onSnapshot:", err);
  });
}
...
function userLoginDictionary() {
  ...
  // Salviamo i dati in registrations (solo info)
...
function bookSlot(fieldName, slot) {
  ...
  db.collection("reservations").doc(docId).set({
    ...
    .then(() => showNotification(`Prenotazione su ${fieldName} alle ${slot} fatta!`))
    .catch(err => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button onclick="updateDictionaryPassword('${username}')">
          <i class="fas fa-edit"></i> Cambia Pass
        </button>
      </td>
    `;
    tbody.appendChild(tr);
...
function saveAdminNotes() {
  if (!isAdmin) {
    showNotification("Non hai permessi per modificare le note!");
    return;
  }
...
function loadAdminImagesRealtime() {
  db.collection("admin").doc("images").onSnapshot(doc => {
    ...
    if (doc.exists) {
      ...
      // Creiamo doc "images" con campi vuoti
      const initData = {};
      for (let i = 1; i <= 10; i++) {
