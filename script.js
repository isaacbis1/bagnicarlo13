...
async function pagaPrenotazione() {
    const username = document.getElementById("username-pay").value;
    if (!username) {
        alert("Inserisci il tuo nome per prenotare!");
        return;
    }
    ...
    try {
        ...
        // Dopo il pagamento, salvare la prenotazione su Firestore
        await db.collection("prenotazioni").add({
            username: username,
            data_partita: new Date(new Date().setDate(new Date().getDate() + 1)), // Prenotazione per domani
            pagato: true
        });
