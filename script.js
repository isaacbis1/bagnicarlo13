...
function authenticateUser(username) {
    ...
    // Mostra il pulsante di pagamento solo dopo il login
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (payButton) {
        payButton.addEventListener("click", pagaPrenotazione);
    }
});

// Funzione per avviare il pagamento e prenotare
async function pagaPrenotazione() {
    console.log("🔵 Bottone 'Prenota con Pagamento' cliccato!");

    const username = document.getElementById("username-pay").value.trim();
    if (!username) {
        alert("⚠️ Inserisci il tuo nome per prenotare!");
        return;
    }

    try {
        console.log("🟡 Invio richiesta a Stripe...");

        const response = await fetch("https://tuo-server.com/create-checkout-session", {
            method: "POST",
        ...
        console.log("🟢 Sessione ricevuta da Stripe:", session);

        if (!session.id) {
            throw new Error("❌ Session ID non ricevuto");
        }

        // Reindirizzamento a Stripe per il pagamento
        await stripe.redirectToCheckout({ sessionId: session.id });

        console.log("✅ Reindirizzamento a Stripe completato!");
...
// Eliminazione delle prenotazioni scadute
async function pulisciPrenotazioni() {
    const oggi = new Date();
    oggi.setHours(0, 0, 0, 0); // Reset dell'orario per controllare solo la data

    try {
        const snapshot = await db.collection("prenotazioni").get();
        snapshot.forEach(async doc => {
            const dataPrenotazione = doc.data().data_partita.toDate();
            if (dataPrenotazione < oggi) {
                await db.collection("prenotazioni").doc(doc.id).delete();
...
// Esegui la pulizia all'avvio della pagina
pulisciPrenotazioni();
